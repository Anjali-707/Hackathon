...
.pill.high {background: rgba(255, 99, 71, .15);} 
.pill.med {background: rgba(255, 206, 86, .15);} 
.pill.low {background: rgba(75, 192, 192, .15);}  
...
def recommend_role(user_skills: List[str]) -> str:
    """Find a role that best matches user skills."""
    best_role, best_score = None, -1
    have = {s.strip().title() for s in user_skills if s.strip()}
    for role, data in ROLE_DB.items():
        req_skills = set([s.title() for s in data.get("skills", [])])
        score = len(have & req_skills)
        if score > best_score:
            best_score = score
            best_role = role
    return best_role or "N/A"

def generate_markdown_report(name: str, current_role: str, years: float, target_role: str, recommended_role: str, roadmap: List[str], gaps: List[Tuple[str,str,int]], recs: List[str]) -> str:
...
**Recommended Role (based on skills):** {recommended_role}
...
recommended_role = recommend_role(skills_list)
...
with col2:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<h3>🧭 Recommended Role (based on skills)</h3>", unsafe_allow_html=True)
    st.success(f"{recommended_role}")
    st.markdown("</div>", unsafe_allow_html=True)
...
report_md = generate_markdown_report(name, current_role, years_exp, desired_role, recommended_role, roadmap_steps, missing, recs)
